namespace ExampleProjLeesAlgorith
{
    internal class Program
    {
        ...
        private static bool IsValid(int[][] matrix, bool[][] visited, int row, in int col)
        {
            return (row >= 0) &&(row < 10) && (col >= 0) && (col < 10) && matrix[row][col] == 1 && !visited[row][col];
        }

        private static void LeesAlgorithm(int[][] matrix, int i, int j, int x, int y)
        {
            ...
            {
                Console.WriteLine("The shortest path from source to destination has length: " +  minimumDistance);
            }
            else
            {
                Console.WriteLine("Destination can't be reached from the given source");
            }
        }
